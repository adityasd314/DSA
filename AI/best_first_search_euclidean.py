...
def euclidean_heuristic(L1, L2):
    return round(euclidean_distance(L1, L2), 2)


def best_first_search(graph, start, end):
    closeList = []
    openList = [[euclidean_heuristic(start, end), start, -1]]
    heapify(openList)
    data = []
    ...
    while openList:
        ...
        for node in successor(top):
            if not any(node == c_node for c_node, _, _ in closeList):
                heappush(openList, [euclidean_heuristic(node, end), node, top])
